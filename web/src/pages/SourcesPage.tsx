...
export default function SourcesPage() {
  ...
  useEffect(() => {
    api.sources().then(d => setSources(d.sources)).catch(() => setSources([])).finally(() => setLoading(false));
  }, []);
            ...
            <div key={s.name} className="glass-card p-5 animate-slide-up" style={{ animationDelay: `${i * 60}ms` }}>
              <div className="flex items-start justify-between gap-4">
                ...
                <a href={s.base_url} target="_blank" rel="noopener noreferrer"
                  className="shrink-0 p-2 rounded-lg hover:bg-muted transition-colors" aria-label={`Visit ${s.name}`}>
