    ...
    <div className="max-w-2xl mx-auto py-8 space-y-8">
      ...
      <div className="glass-card p-6 space-y-5 animate-slide-up">
        <div className="space-y-2">
          ...
          <input type="number" value={age} onChange={e => setAge(e.target.value)} min="1" max="150"
            placeholder="e.g. 21" className="w-full px-4 py-3 rounded-xl bg-muted text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
        </div>
        ...
        <div className="flex items-center gap-3">
          <input type="checkbox" id="epic" checked={hasEpic} onChange={e => setHasEpic(e.target.checked)}
            className="w-4 h-4 rounded border-border accent-primary" />
          <label htmlFor="epic" className="text-sm">I have a Voter ID (EPIC) card</label>
        </div>

        <button onClick={check} disabled={loading || !age}
          ...
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><span>Check Eligibility</span><ArrowRight className="w-4 h-4" /></>}
        </button>
      </div>
      ...
      {result && (
        <div className={`glass-card p-6 animate-slide-up ${result.eligible ? 'border-secondary/50' : 'border-destructive/50'}`}>
          <div className="flex items-center gap-3 mb-4">
            {result.eligible ? <CheckCircle2 className="w-8 h-8 text-secondary" /> : <XCircle className="w-8 h-8 text-destructive" />}
