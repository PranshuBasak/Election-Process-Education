    ...
    <div className="max-w-3xl mx-auto py-8 space-y-8">
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="glass-card px-5 py-6 h-16 animate-pulse">
          ...
          {filtered.map((t, i) => (
            <div key={t.term} className={`glass-card overflow-hidden animate-slide-up transition-all duration-300 border ${expanded === t.term ? 'border-primary/30 shadow-md bg-card/80' : 'border-transparent hover:border-border'}`} style={{ animationDelay: `${i * 40}ms` }}>
              <button 
                ...
                aria-expanded={expanded === t.term}
                aria-controls={`glossary-content-${t.term}`}
                ...
                <span className={`text-muted-foreground transition-transform duration-300 bg-muted p-1 rounded-full ${expanded === t.term ? 'rotate-180 bg-primary/10 text-primary' : ''}`}>
                  <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.13523 6.15803C3.3241 5.95657 3.64052 5.94637 3.84197 6.13523L7.5 9.56464L11.158 6.13523C11.3595 5.94637 11.6759 5.94637 11.8648 6.15803C12.0536 6.35949 12.0434 6.67591 11.842 6.86477L7.84197 10.6148C7.64964 10.7951 7.35036 10.7951 7.15803 10.6148L3.15803 6.86477C2.95657 6.67591 2.94637 6.35949 3.13523 6.15803Z" fill="currentColor" fillRule="evenodd" clipRule="evenodd"></path></svg>
                </span>
              </button>
              
              <div 
                id={`glossary-content-${t.term}`}
                role="region"
                className={`grid transition-all duration-300 ease-in-out ${expanded === t.term ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0 overflow-hidden'}`}>
